...
Original file is located at
    https://colab.research.google.com/drive/1DuXwKFnNljUpI9NiRqL1WGtBK4XMeLJa
"""
...
def del_after_use():
    ...
    folder = "D:\ExcelR\project_app\Resumes"
...
def extract_name(resume_text):
    ...
    #     # First name and Last name are always Proper Nouns
        ...
        for match_id, start, end in matches:
             ...
             return span.text
    #df = pd.read_csv(r"C:\Users\Sandhya Sudi\Downloads\Features_details.csv")
...
def extract_university(df1):
    uni_df = pd.read_csv(r"C:\Users\Sandhya Sudi\Desktop\List of Universities.csv")
    uni_df = uni_df.iloc[:,1:]
    f_uni = []

    for k in range(len(df1)):    # Loop to input Resumes one at a time
        university_name = []
        lines = sent_tokenize(df1['Resume Text'][k])          # Tokenizing text into sentences
        for sentence in lines:                               # Loop to run through each sentence
                ...
                # print(sentence)
                    if re.search(r'\s{3,5}',sentence):  # If sentence has more than 3 spaces between words then split them
                        ...
                        edu_sen = sentence
                        for j in df1['Name of University']:      # Loop to run through each university name
                            if re.search(j.lower(),edu_sen.lower()):
                                university_name.append(j)           # If university is found, append it.
...
def extract_info(text, filename):
    ...
    if filename.lower().endswith('.zip'):
        ...
        st.write(education)
        university = extract_university(df1=[])
